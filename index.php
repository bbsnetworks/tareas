...
<body class="dark text-white">
  ...
  <div class="container mx-auto mt-16 mb-4">
    ...
    <form id="eventForm" class="space-y-4 p-4 rounded relative z-10">
      ...
      <div>
        ...
        <input type="datetime-local" id="start" class="w-full p-2 border border-gray-600 rounded bg-gray-900 text-white"
          required>
      </div>
      <div>
      ...
      <div>
        ...
        <input type="color" id="color" class="w-full p-2 border border-gray-600 rounded bg-gray-900 text-white"
          value="#38bdf8">
      </div>
      <div>
        <label for="categoria" class="block text-sm font-medium">Categoría</label>
        <select id="categoria" class="w-full p-2 border border-gray-600 rounded bg-gray-900 text-white" required>
      ...
      <div>
        ...
        <input type="text" id="here-autocomplete"
          class="w-full p-2 border border-gray-600 rounded bg-gray-900 text-white"
          placeholder="Selecciona una ubicación" readonly>
    ...
    <div id="eventModal" class="hidden fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center">
      <div class="bg-gray-800 rounded-lg shadow-lg p-6 relative modal">
        <div id="eventDetails" class="transition-transform duration-500 transform translate-x-0">
          ...
          <button id="closeModalButton" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 mt-4">Cerrar
            <i class="bi bi-box-arrow-left"></i></button>
          <button id="deleteEventButton" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">Eliminar <i
              class="bi bi-trash"></i></button>
        </div>
        <div id="sliderContainer"
          class="hidden transition-transform duration-500 transform translate-x-full absolute top-0 left-0 w-full h-full p-6 bg-gray-800">
          ...
          <div class="mt-2">
            ...
            <input type="file" id="evidence" capture accept="image/*" multiple
              class="mt-1 block w-full border border-gray-600 rounded-md shadow-sm bg-gray-900 text-white">
          </div>
          <div class="mt-4">
            ...
            <textarea id="comments" rows="3"
              class="mt-1 block w-full border border-gray-600 rounded-md shadow-sm p-2 bg-gray-900 text-white"
  ...
  <button id="openDayTasks"
    class="fixed right-6 bottom-6 z-50 w-14 h-14 rounded-full bg-cyan-500 hover:bg-cyan-400 text-white shadow-xl flex items-center justify-center transition transform hover:scale-105 active:scale-95"
  ...
  <div id="dayTasksModal" class="hidden fixed inset-0 z-50">
    ...
    <div class="relative mx-auto mt-24 w-[95%] max-w-3xl">
      <div class="bg-gray-800 rounded-xl border border-gray-700 shadow-2xl overflow-hidden">
        <div class="flex items-center justify-between p-4 border-b border-gray-700">
          <div>
            <h3 class="text-lg font-bold">Tareas del día</h3>
            <p class="text-sm text-gray-300" id="dayTasksSubtitle">Selecciona una fecha para ver la lista</p>
          </div>

          <button id="closeDayTasks"
            class="w-10 h-10 rounded-lg bg-gray-700 hover:bg-gray-600 flex items-center justify-center">
        ...
        <div class="p-4 space-y-4">
          ...
          <div class="flex flex-col md:flex-row gap-3 md:items-end">
            <div class="flex-1">
              <label class="block text-sm font-medium text-gray-200 mb-1">Día</label>
              <input id="dayTasksDate" type="date"
                class="w-full p-2 border border-gray-600 rounded bg-gray-900 text-white" />
            </div>

            <div class="flex-1">
              <label class="block text-sm font-medium text-gray-200 mb-1">Buscar</label>
              <input id="dayTasksSearch" type="text" placeholder="Ej: reunión, llamada, Uriangato..."
                class="w-full p-2 border border-gray-600 rounded bg-gray-900 text-white" />
            </div>

            <div class="flex gap-2">
              <button id="refreshDayTasks" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">
          ...
          <!-- Lista -->
          <div id="dayTasksList" class="max-h-[60vh] overflow-auto pr-1 space-y-3">
            <!-- aquí se inyecta la lista -->
          </div>
        </div>

        <div class="p-4 border-t border-gray-700 flex justify-end gap-2">
          <button id="closeDayTasksFooter" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded">
            Cerrar
          </button>
        </div>
      </div>
